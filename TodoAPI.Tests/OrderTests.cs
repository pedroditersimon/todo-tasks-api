...
public class OrderTests
{
	...
	public async Task OrderTasksByFavoriteAndCompleted()
	{
		...
		// Act: mark the task as favorite and completed
		task2.IsFavorite = true; // Mark task2 as favorite
		task3.IsCompleted = true; // Mark task3 as completed
		await unitOfWork.TaskService.Update(task2);
		await unitOfWork.TaskService.Update(task3);
		...
		Assert.NotNull(tasks);
		Assert.Equal(3, tasks.Count);
		Assert.Equal(task2.ID, tasks[0].ID); // Verify that task2 (favorite) is first
		Assert.Equal(task3.ID, tasks[1].ID); // Verify that task3 (completed) is second
		Assert.Equal(task1.ID, tasks[2].ID); // Verify that task1 (not completed, not favorite) is last
	}
	...
	public async Task OrderGoalsByFavoriteAndCompleted()
	{
		...
		// Act: mark the goal as favorite and completed
		goal2.IsFavorite = true; // Mark goal2 as favorite
		goal3.IsCompleted = true; // Mark goal3 as completed
		await unitOfWork.GoalService.Update(goal2);
		await unitOfWork.GoalService.Update(goal3);
		...
		Assert.NotNull(goals);
		Assert.Equal(3, goals.Count);
		Assert.Equal(goal2.ID, goals[0].ID); // Verify that goal2 (favorite) is first
		Assert.Equal(goal3.ID, goals[1].ID); // Verify that goal3 (completed) is second
		Assert.Equal(goal1.ID, goals[2].ID); // Verify that goal1 (not completed, not favorite) is last
	}
